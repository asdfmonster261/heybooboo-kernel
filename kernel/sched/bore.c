...
static_assert(sizeof(struct bore_ctx) == 32,
	"bore_ctx must stay 32 bytes to fit KABI reserve slots 4-7");
#include "sched.h"
...
static void reweight_task_by_prio(struct task_struct *p, int prio) {
	...
	if (se->on_rq) {
		task_bore(p)->stop_update = true;
		reweight_entity(cfs_rq_of(se), se, weight);
		task_bore(p)->stop_update = false;
	} else
...
static void update_penalty(struct task_struct *p) {
	struct bore_ctx *ctx = task_bore(p);
...
static void update_burst_cache(struct bore_bc *bc,
	...
	struct bore_bc new_bc = {
		.penalty = max(average, task_bore(p)->penalty),
...
static u32 inherit_from_parent(struct task_struct *parent,
	...
	struct bore_bc *bc = &task_bore(parent)->subtree;
...
static u32 inherit_from_ancestor_hub(struct task_struct *parent,
	...
	struct bore_bc *bc = &task_bore(ancestor)->subtree;
	...
	if (burst_cache_expired(bc, now)) {
		...
		for_each_child_task(ancestor, direct_child) {
			...
			count++;
			total += task_bore(descendant)->penalty;
		}
...
static u32 inherit_from_thread_group(struct task_struct *p, u64 now) {
	...
	struct bore_bc *bc = &task_bore(leader)->group;
	...
	if (burst_cache_expired(bc, now)) {
		...
		for_each_thread(leader, sibling) {
			...
			count++;
			total += task_bore(sibling)->penalty;
		}
...
{ memset(task_bore(p), 0, sizeof(struct bore_ctx)); }
